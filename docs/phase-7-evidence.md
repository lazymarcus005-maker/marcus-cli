...
- Build and tests: `npm test` (146 passed, 0 failed), `npm run typecheck`, and `npm run build` passed with Node.js 24.21.0 on macOS arm64 on 2026-09-19. The same checks were rerun in the Linux arm64 container against commit `4638f0f` below.
...
- Linux arm64 validation: a pinned Node 24.21.0 Debian Bookworm container completed clean install, all 146 tests, typecheck, and build against commit `4638f0f`. The packed tarball then installed into a clean prefix with `CXXFLAGS=-std=c++20` for Tree-sitter 0.21; packaged CLI help/version smoke checks passed and `npm audit --omit=dev` reported zero vulnerabilities. npm reported unapproved dependency install scripts that it skipped; the image included `git`, compiler tools, Python, and `ripgrep` for tests/build/runtime.
