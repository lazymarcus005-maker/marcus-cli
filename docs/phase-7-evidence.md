...
- Build and tests: `npm test` (146 passed, 0 failed), `npm run typecheck`, and `npm run build` passed with Node.js 24.21.0 on 2026-09-19. The Linux arm64 evidence below predates twenty-one test additions made since that run; no Linux rerun was performed for these additions.
