...
- Build and tests: `npm test` (137 passed, 0 failed), `npm run typecheck`, and `npm run build` passed with Node.js 24.21.0 on 2026-09-19. The Linux arm64 evidence below predates twelve test additions made since that run; no Linux rerun was performed for these additions.
...
- The paired harness and a partial in-process Pi SDK baseline adapter exist, but no Macus adapter, controlled Macus-versus-unmodified-Pi measurements, or quality-regression report exist. See [benchmark-protocol.md](benchmark-protocol.md).
