...
- Build and tests: `npm test` (152 passed, 0 failed), `npm run typecheck`, and `npm run build` passed with Node.js 24.21.0 on macOS arm64 on 2026-09-19.
...
- Linux arm64 validation: a pinned Node 24.21.0 Debian Bookworm container completed clean install, all 150 tests, typecheck, and build against commit `abfda70`. `npm ci` and `npm audit --omit=dev` reported zero vulnerabilities. The packed tarball then installed into a clean prefix with `CXXFLAGS=-std=c++20` for Tree-sitter 0.21; packaged CLI help/version smoke checks passed. npm reported unapproved dependency install scripts that it skipped; the image included `git`, compiler tools, Python, and `ripgrep` for tests/build/runtime. This validates the release snapshot before the latest recovery-boundary change; that change still needs an updated Linux run.
