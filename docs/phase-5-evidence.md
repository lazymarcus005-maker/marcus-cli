...
- Issue #19 still lacks a full process-restart matrix for durable execution completion before Pi transcript completion and external-effect reconciliation. `/recovery` now surfaces bounded redacted intent and effect classes, but there is no in-session resolution command; operators must inspect provider/worktree state manually before using `/clear`, and the crash/restart scenarios remain incomplete.
