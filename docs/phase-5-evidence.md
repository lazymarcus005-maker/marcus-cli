...
- Cancellation during an active compaction request and compaction crash-point tests remain open; external-effect reconciliation is still manual.
- Issue #19 still lacks a full process-restart matrix. The durable-completion-before-Pi-transcript-result gap is covered and pauses safely, but other transcript/journal reconciliation and external-effect scenarios remain incomplete. `/recovery` surfaces bounded redacted intent and effect classes, but there is no in-session resolution command; operators must inspect provider/worktree state manually before using `/clear`.
