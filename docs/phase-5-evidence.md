...
- State-store and execution tests cover several recovery boundaries: cancellation before authorization and while authorization is pending do not prepare journal entries or spawn; cancellation after durable intent but during asynchronous log allocation records a confirmed prelaunch failure and does not spawn; post-launch journal failure kills and observes the process group; a subprocess crash leaves a `prepared` execution and a `started` execution after its child command changes the workspace, and resume converts both to `unknown`; a completed side effect whose result persistence fails remains `unknown` after database reopen; duplicate completed tool-call IDs are refused after database reopen; and injected migration failure rolls back and can be retried. Additional tests cover ledger/checkpoint recovery, branch/HEAD mismatch, unknown/cancelled-effect gates across restart, checkpoint reconciliation, and single-writer locks. This is partial evidence for issue #19, not the complete interruption matrix; cross-store crash gaps remain listed below. The pinned Node.js 24.21.0 full suite passes 150 tests; typecheck, build, and CLI smoke checks pass.
...
- Issue #19 still lacks a full process-restart matrix for durable execution completion before Pi transcript completion and external-effect reconciliation. One real crash boundary is now covered. `/recovery` surfaces bounded redacted intent and effect classes, but there is no in-session resolution command; operators must inspect provider/worktree state manually before using `/clear`, and the broader crash/restart scenarios remain incomplete.
