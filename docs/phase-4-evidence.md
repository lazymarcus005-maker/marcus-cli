...
- Git context reports branch/HEAD, bounded tracked diff, changed paths, and a digest that includes dirty/untracked file bytes. New durable sessions atomically record their initial dirty paths as `pre_existing`; agent writes record old/new hashes, and shell-observed changes are labelled `external_or_unknown`.
...
Focused verification under Node.js 24.21.0 passes; the full suite currently passes 129 tests. Typecheck, build and CLI help/version smoke checks pass. Live endpoint and cross-platform gates still remain.
