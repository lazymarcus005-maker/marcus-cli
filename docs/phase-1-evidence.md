...
| --- | --- | --- |
| Current automated suite | `npm test` | 121 passed, 0 failed (2026-09-19); deterministic local providers only |
| Pi protocol and recovery checks | `npx tsx --test test/pi-runtime.test.ts` | 19 passed, 0 failed; nested tool-call and tool-result IDs form a correctly ordered protocol group; streaming, cancellation, compaction, dispose/replacement, and resume checks run against the deterministic local provider |
...
- The paired benchmark harness is implemented and unit-tested, but baseline adapters and controlled measurements are still absent; no performance claims are made. See `docs/benchmark-protocol.md`.
