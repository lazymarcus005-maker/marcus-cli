...
| --- | --- | --- |
| Current automated suite | `npm test` | 150 passed, 0 failed (2026-09-19); deterministic local providers only |
...
- Complete centralized read/write/tool authorization and bounded-output policy; a subprocess crash after a workspace side effect but before completion persistence is now covered and converted to an `unknown` execution on resume. Broader crash-point reconciliation and complete user-change preservation remain open.
...
- A real subprocess-crash test leaves both a `prepared` execution and a workspace-changing `started` execution unfinished; on resume, both are transactionally changed to `unknown` and remain recovery-blocking.
...
- The paired benchmark harness plus partial in-process Pi and Macus adapters are implemented and locally tested. Metric distributions remain separate for each task, system, and repository/provider cache condition, including explicit unknown states. The deterministic reference-fixture generator produces the exact 10,000-file, 100 MiB mixed-language corpus and a hashed manifest. Both adapters apply validated OpenAI-compatible scenario generation settings, report observed loopback token usage, preserve unknown metrics when usage is omitted, and treat failed prompts as unknown correctness/recovery. Their in-process form does not measure isolated CLI RSS/first-useful-edit time. No live endpoint or controlled paired measurements are available; no performance claims are made. See `docs/benchmark-protocol.md`.

The Pi adapter disables built-in tools and registers Macus policy-controlled tools. Local provider tests prove nested tool-call turns and session disposal/replacement; logs are spooled and checkpoint files reconciled. The subprocess crash test covers one write-before-completion persistence boundary; full interruption/crash reconciliation, baseline evidence, and actual configured endpoint compatibility remain incomplete, so this is not a Phase 1 pass.
