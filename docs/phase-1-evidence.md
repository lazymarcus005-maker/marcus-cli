...
| --- | --- | --- |
| Current automated suite | `npm test` | 146 passed, 0 failed (2026-09-19); deterministic local providers only |
...
- A real subprocess-crash test writes a workspace file and terminates the agent process before execution completion is journaled; on resume, the durable `started` execution is transactionally changed to `unknown` and remains recovery-blocking.
