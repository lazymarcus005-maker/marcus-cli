...
- Configure an authorized endpoint for live compatibility evidence; current provider testing is loopback-only, and this is now the principal Phase 1 gate blocker. Current execution/log unit tests cover 8 MiB in-memory / 100 MiB spool limits, bounded session-scoped log access, retention and aggregate limits.
