...
describe("bounded close-out review report", () => {
  it("reports changed, tested, risk, and unresolved issue sections without overstating evidence", () => {
    ...
    const report = formatReviewReport({
      context,
      evidence: [{ evidenceId: "ev-1", snapshotDigest: "old", status: "passed", payload: { command: "npm test", durationMs: 52, tests: 3, passed: 3, failed: 0, parserStatus: "parsed", exitCode: 0, signal: null, timedOut: false, cancelled: false, outputComplete: true, logRef: "log-1" } }],
  ...
  it("surfaces blocked tasks as unresolved and labels missing tests as a risk", () => {
    ...
    assert.match(report, /No test evidence/);
